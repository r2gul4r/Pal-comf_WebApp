...
def case_two_story() -> list:
    story = page_heading(
        ...
        "작업 필터가 의도와 다르게 동작",
        "선택한 작업과 관계없는 팰이 검색 결과에 함께 노출되는 상황이 발생했습니다. 원본 필드의 의미와 대상 팰의 작업 적성을 다시 확인하고 전체 효과 기준으로 적용 범위를 구조화했습니다.",
        ORANGE,
    )
    story += chat(
        "사용자 - 예상과 다른 결과 확인",
        "디자인은 좋은데 기능면에서 이상한 점이 있음. 선택한 작업과 관계없는 팰이 검색 결과에 왜 나옴.",
        "user",
        "실제 작업 대화 요약",
    ...
    story += chat(
        "AI - 원인 분석",
        "원인 잡힘. 개별 데이터가 틀린 게 아니라 필터가 `작업 종류가 null인 효과`를 모든 작업에 무조건 포함시키는 조건이라 결과가 새는 거임. UI가 효과 대상 팰의 실제 작업 적성까지 확인하지 않고 있음.",
        "ai",
        "실제 작업 대화 요약",
    )
    story += chat(
        "사용자 - 대응 범위 결정",
        "한 건만 고치지 말고 전체적으로 검수하고 패치해.",
        "user",
        "실제 작업 대화 요약",
    ...
    story += chat(
        "AI - 전체 데이터 확인 결과",
        "26개 중 `workSuitabilityId=null`은 8개인데, 7개는 진짜 전 작업 특성이고 1개는 특정 대상 한정 효과였음. null 하나로 둘을 뭉갠 게 버그라 추출 데이터부터 작업 적용 범위를 명시하는 식으로 고치는 중임.",
        "ai",
        "실제 작업 대화 요약",
    )
    story += [
        three_cards(
            [
                (
                    "발생한 상황",
                    "원본 필드에 특정 작업이 없는 효과가 모든 작업의 결과에 포함되면서, 선택 조건과 관계없는 팰까지 노출됐습니다.",
                    RED,
                ),
                (
                    "대응 방법",
                    "효과 대상 팰의 설치본 작업 적성을 읽어 효과별 applicableWorkSuitabilityIds를 추출 단계에서 계산했습니다.",
                    BLUE,
                ),
                (
                    "검증 결과",
                    "효과 26개와 작업 필터 12종을 전수 검사하고, 선택한 작업과 관계없는 결과가 더 이상 노출되지 않는 것을 확인했습니다.",
                    GREEN,
                ),
            ]
        ),
...
def closing_story() -> list:
    return [
        *page_heading(
            "RETROSPECTIVE",
            "이번 작업에서 내가 느낀 점",
            "이번 작업에서 가장 잘됐다고 느낀 점은 내가 불편했던 점을 확실하게 해결한 것이다.",
            GREEN,
        ),
        Paragraph("실제로 작업하며 느낀 부분", STYLES["section"]),
        three_cards(
            [
                (
                    "1. 잘됐다고 느낀 점",
                    "정식 버전의 새 패시브와 스킬을 팰별로 하나씩 확인하지 않고, 키워드 하나로 필요한 팰을 바로 찾게 만들었다.",
                    CYAN,
                ),
                (
                    "2. 굳이 고른 막힌 지점",
                    "중간중간 크게 막히는 부분은 없었다. 굳이 하나를 고르면 패시브 UI, 아이콘, 카드와 자막을 조율한 디자인 부분이었다.",
                    BLUE,
                ),
                (
                    "3. AI 결과를 검수한 방식",
                    "AI가 도출한 결과를 그냥 '아 그렇구나' 하고 적용하지 않았다. 왜 그렇게 했는지, 다른 더 좋은 방법은 없는지, 지금 방법이 제대로 된 건지를 계속 질문하고 검수하면서 작업했다.",
                    GREEN,
                ),
            ]
        ),
        ...
        card(
            "실패 아카이브 만들기",
            "중간중간 막히는 부분이 크게 없었지만, 만약 더 복잡한 작업을 했을 땐 모든 막혔던 지점을 기록하고 시도했던 방법을 일단 기록하여 실패 아카이브를 만들고 다음 작업 때 비슷한 상황이 나왔을 때 참고할 수 있도록 할 것 같다.",
            GREEN,
        ),
        Spacer(1, 10),
        card(
            "해결 흐름을 다시 사용하기",
            "AI가 틀리거나 이상하게 작업할 시 지침이 없다면 간단하게 지침을 세우고 정확하게 작업할 수 있도록 세세하게 설명하면서 차근차근 문제를 풀어간 뒤, 해결이라는 방향으로 성공적으로 흘러간 해당 흐름을 메모리에 저장할 수 있도록 문서화시켜서 다시 사용할 수 있도록 하는 방식을 사용할 것 같다.",
            CYAN,
        ),
        Spacer(1, 10),
        Paragraph(
